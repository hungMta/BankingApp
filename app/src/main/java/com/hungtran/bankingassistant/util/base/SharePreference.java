...
public class SharePreference {
    ...
    private static SharePreference getInstance() {
        if (instance == null) {
    ...
    private SharePreference(Context context, String sharePreferencesName) {
        prefs = context.getApplicationContext().getSharedPreferences(sharePreferencesName, Context.MODE_PRIVATE);
    }

    public static SharePreference initHelper(Context context) {
        if (instance == null)
            instance = new SharePreference(context);
        return instance;
    }

    public static SharePreference initHelper(Context context, String sharePreferencesName) {
        if (instance == null)
            instance = new SharePreference(context, sharePreferencesName);
        return instance;
    }
    ...
    public static <K, V> Map<K, V> getMapVal(String KEY) {
        ...
        try {
            String obj = getInstance().prefs.getString(KEY, "");
            objects = new Gson().fromJson(obj, new TypeToken<Map<K, V>>() {
            }.getType());
        } catch (Exception ex) {
            ex.printStackTrace();
        }
